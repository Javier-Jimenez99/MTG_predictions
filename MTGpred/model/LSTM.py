...
class DeckEncoder(nn.Module):
    def __init__(
        ...
        self.transformer = AutoModelForSequenceClassification.from_pretrained(
            base_model, num_labels=transformer_output_size
        )
        self.LSTM = nn.LSTM(
            input_size=100,
...
def train(
    ...
    # squeduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
    #    optimizer, T_0=500, T_mult=2, eta_min=1e-6, last_epoch=-1
    # )
    ...
    for epoch in range(epochs):
        ...
        train_losses = []
        train_accuracies = []
        for deck1, deck2, mask1, mask2, target in tqdm(train_dataloader, desc="TRAIN"):
